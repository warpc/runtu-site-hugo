  ...
  module Importers
    class Joomla < Importer
      ...
      def self.process(options)
        ...
        #query = "SELECT * FROM #{table_prefix}content WHERE (state = '0' OR state = '1') AND sectionid = '#{section}'"
        ...
        db[query].each do |post|
         # puts post.inspect
          ...
          data = {
            ...
            "draft"      => false,
            "slug"       => post[:alias],
            "aliases"    => ["/news-project/#{post[:alias]}.html"]
          ...
          File.open("_posts/#{name}", "w") do |f|
            ...
            f.puts "---"
            f.puts ""
            f.puts ReverseMarkdown.convert(content, unknown_tags: :bypass)
          end
        end
      end
